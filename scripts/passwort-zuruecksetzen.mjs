...
const r = await q("UPDATE maisdoc.users SET pw_hash = $2 WHERE email = $1 RETURNING id", [email, hashPassword(pw)]);
if (!r.rows[0]) { console.error("Kein Konto mit dieser E-Mail."); process.exit(1); }
await q("DELETE FROM maisdoc.sessions WHERE user_id = $1", [r.rows[0].id]);
